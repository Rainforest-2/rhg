...
| Spirit | `DataUnit.ints[110]`, `StageBasis`, `LineUp`, `EUnit` | Spirit is production/stage state, not an ordinary proc status. It has cooldown, one-spirit-per-summoner, attack-on-add, damage rejection, and self-removal after attack. | Runtime and attack-only bundle path exist; actor/A_IMUATK visual acceptance pending. |
...
| Combo/orb/treasure/talent/PCoin | `BasisLU`, `LineUp`, `Treasure`, `EUnit/EEnemy`, `PCoin`, attack construction | Modifiers originate in basis/lineup/entity construction and can affect stats, damage, proc payloads, resistance, and traits. | Core hooks exist; broad real-data and browser acceptance remain partial. |
