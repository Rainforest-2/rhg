...
| Combo/orb/treasure/talent/PCoin | `BasisLU`, `LineUp`, `Treasure`, `EUnit/EEnemy`, `PCoin`, attack construction | Modifiers originate in basis/lineup/entity construction and can affect stats, damage, proc payloads, resistance, and traits. `EUnit.getAbi()` grants `AB_VKILL` from `C_VKILL` when the active combo increment is positive. BCU cat deploy cost starts from `DataUnit.price`, then `Form/EForm.getPrice(sta)` applies `price * (1 + sta * 0.5)`; default `StageMap.price=1` makes ordinary deploy cost 1.5x raw price before internal `ELineUp.price = 100 * ...`. | Core hooks exist; real-data sweeps cover combo/orb/treasure/talent/PCoin and `C_VKILL` now reaches the damage resolver. Browser acceptance remains separate. |
