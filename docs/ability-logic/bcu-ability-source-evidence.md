...
| Spirit | `DataUnit.ints[110]`, `StageBasis` (spiritCooldown/spiritEmphasize/unitRespawnTime/shock loop), `LineUp`, `EUnit` | Spirit is production/stage state, not an ordinary proc status. It has a once-per-frame cooldown countdown, a cooldown-ready emphasize cue, one-spirit-per-summoner, pre-warp summon origin, side-capacity gating, a one-frame post-conjure `unitRespawnTime` production lock, attack-on-add, damage rejection, boss-shockwave immunity, and self-removal after attack. | Runtime, attack-only bundle path, cooldown/emphasize, post-conjure production lock, card-layer ready-state wiring, and shockwave immunity are tested; actor/A_IMUATK and conjure-card flash visual acceptance pending. |
| Boss shockwave / shockwave immunity | `StageBasis` shock loop, `Entity.interrupt(INT_SW)`, `DataUnit.ints[56] -> AB_IMUSW` | Boss-spawn shockwave interrupts normal touchable player units, but `INT_SW` interruption is rejected when the target has `AB_IMUSW`; spirits are also excluded from the shock loop. | Runtime now skips `AB_IMUSW` actors in `BattleBossShockwaveRuntimePatch`; visual effect acceptance remains separate. |
...
| Combo/orb/treasure/talent/PCoin | `BasisLU`, `LineUp`, `Treasure`, `EUnit/EEnemy`, `PCoin`, attack construction, `ELineUp` | Modifiers originate in basis/lineup/entity construction and can affect stats, damage, proc payloads, resistance, traits, price, and cooldown. `EUnit.getAbi()` grants `AB_VKILL` from `C_VKILL` when the active combo increment is positive. BCU cat deploy cost starts from `DataUnit.price`, then `Form/EForm.getPrice(sta)` applies `price * (1 + sta * 0.5)`; default `StageMap.price=1` makes ordinary deploy cost 1.5x raw price before internal `ELineUp.price = 100 * ...`. `PCoin.improve` applies `PC2_COST` as `price -= value` and `PC2_CD` as `respawn -= value`; `ELineUp` then applies C_DISCOUNT to price and `Treasure.getFinRes(respawn, C_RESP)` to production cooldown, with max LV_RES=30/T_RES=300 yielding the 60F floor for short-respawn units such as Tank Cat. | Core hooks exist; real-data sweeps cover combo/orb/treasure/talent/PCoin, `C_VKILL` reaches the damage resolver, and production cost/cooldown uses the BCU ELineUp path. Browser acceptance remains separate. |
