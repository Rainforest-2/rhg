...
| spirit lifecycle | `human-visual-review-needed` | Spirit actor spawn, attack animation, A_IMUATK appearance, cleanup after attack | `not-reviewed` | Runtime lifecycle and attack-only semantic ZIP loading checks exist; headless capture `tmp/visual-shots/spirit-728.png` shows spirit 729 spawning from conjurer 728, but human visual acceptance is still not recorded. |
