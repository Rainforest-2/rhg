...
| Spirit | `human-visual-review-needed` | 召喚者と攻撃専用 spirit 形態 | 生成、攻撃アニメ、A_IMUATK、攻撃後の後始末、conjure-card の ready flash | `not-reviewed` | Semantic ZIP / factory 経路はある。ワープ前の起点、容量制限、召喚者が生存中の一回制限、1 フレームごとのクールダウン、cooldown-ready 強調、conjure 後 1 フレームの生産ロック、生産カードの ready 状態接続、boss-shockwave 免疫は決定的チェック済み。`PlayerProductionBar` は `bcuSpirit` を `ProductionCardSkin` に渡し、`ui:battle` の `spiritSummon.imgcut` part[53] で ready 文字を描く。あとはその flash と spirit / A_IMUATK の見た目をブラウザ受け入れするだけ。 |
...
| SUMMON entry | `human-visual-review-needed` | ローダー付きの実カスタム proc-object summon フィクスチャ | `anim_type`、入場フェーズ、配置、レイヤー、same_health / bond_hp の見える影響 | `not-reviewed` | `check-bcu-summon-procobject-loader-parity` で実データの loader → `BattleAttackProfile` → immediate/on_hit spawn まで確認済み。実カスタムパックの自動発見 / 読み込みを理由にした `blocked` は解消済みで、残るのは入場見た目のブラウザ受け入れのみ。 |
