...
| death surge / zombie corpse interaction | `partial` | `scripts/check-bcu-zombie-corpse-soulstrike-parity.mjs` now covers revive indexes, AB_ZKILL/AB_CKILL, revive HP, `REVIVE_SHOW_TIME` corpse targetability, non-soulstrike exclusion, soulstrike cancellation, zombie killer suppression, and death surge single-spawn at demon soul frame 21. Mini-death-surge and zombie revive visual phase are still not fully proven. | Implement mini-death-surge only after a loader-backed/custom holder source is proven; add revive visual trace before status upgrade. |
| zombie corpse / soulstrike full parity | `partial` | Corpse targetability and soulstrike cancellation are now deterministic and BCU-timed; full visual revive phase and custom/extra revive interactions remain incomplete. | Add zombie revive visual/effect trace and extra-reviver range tests. |
