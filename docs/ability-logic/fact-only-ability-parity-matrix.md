...
| 蘇生 | `P_REVIVE`; enemy 45/46/47 | death/post-damage; corpse state and revive after time with HP percent | `parseEnemyProc` | `BattleActorZombieRevivePatch`, `BcuZombieCorpseRuntime` | zombie revive visual alias/trace not proven | `partial`; logic/window/HP tests pass, revive visual remains missing |
...
| Zombie corpse / soulstrike / death surge | `DataEnemy` revive 45/46/47, death surge 89-92; `DataUnit` `AB_ZKILL` 52, `AB_CKILL` 98 | `Entity.ZombX`, `Entity.touchable`, `Entity.getTouch`, `AtkModelEntity.getDeathSurge` | revive HP `maxH * health / 100`; `REVIVE_SHOW_TIME`; revive timer plus zombie effect length; death/mini-death surge random range; death surge at demon soul frame 21 | `BcuZombieCorpseRuntime`; `BattleActorZombieRevivePatch`; `BattleSoulstrikePatch`; `BcuDeathAnimationRuntime` | `effect:soul` aliases; zombie revive visual alias not proven | `partial` | mini-death-surge split and zombie revive visual trace remain | `check-bcu-zombie-corpse-soulstrike-parity.mjs` |
