...
| 超賢者特効 status resistance | `EEnemy.getResistValue`, `EUnit.getResistValue`; `SUPER_SAGE_RESIST_TYPE` | proc duration/distance/percent resistance; enemy sage 70% resist unless attacker has `AB_SKILL`; unit `AB_SKILL` reduces sage-source status by 70% | `parseUnitAbility` has `AB_SKILL`; enemy trait 104 sage parsed | `BcuResistRuntime.getBcuResistValue` / `resolveBcuProcResistance`; `BcuProcImmunityPatch` delegates partial math there | no separate visual | `code-complete`; verified by `node scripts/check-proc-immunity-resistance-parity.mjs` |
...
| 爆波 `P_BLAST` | `P_BLAST`; unit 113/114/115, enemy 106/107/108; `AttackSimple.excuse` -> `ContBlast(new AttackBlast(...))`; `AttackBlast.excuse` resets `atk=rawAtk`, applies strengthen/weaken, then applies 100/70/40% level falloff | after captured hit; `ContBlast` attacks from frame 10, level bands reset at 10/20/30; target-specific damage resolved from raw projectile base with falloff once | `parseUnitProc`, `parseEnemyProc`; `AbilityModel`/`ProcResolver` semantic added; `DamageCalculator.bcuProjectileBaseDamage` | `BattleBlastRuntimePatch` queues ContBlast-like runtime from raw projectile base and uses non-recursive blast event | `effect:wave` `unit-blast`/`enemy-blast`; ZIP entries verified by `check-effect-bundle-aliases`; coordinate trace verified by `check-effect-coordinate-traces` | `human-visual-review-needed`; code/effect evidence passes, exact human appearance not manually reviewed |
...
| partial resistances | BCU `getResistValue`, sage hunter, talent/orb fields | supported proc duration/distance/percent reduction uses BCU `1 - procResist/100`; broad talent/orb/other holder sources still require proof | full/partial IMU fields parsed where current combat model exposes them; broader external sources not safely parsed | `BcuResistRuntime` centralizes supported math and `BcuProcImmunityPatch` delegates to it | no distinct visual | `partial`; supported IMU/sage branch is `code-complete`, broader holder source coverage remains missing |
...
- W0 proof harness: added deterministic checks `scripts/check-bcu-parser-indexes.mjs`, `scripts/check-projectile-damage-parity.mjs`, `scripts/check-proc-immunity-resistance-parity.mjs`, `scripts/check-effect-bundle-aliases.mjs`, `scripts/check-effect-coordinate-traces.mjs`, and `scripts/check-debug-allocation-guards.mjs`.
- W1 projectile damage basis: BCU references used: `AttackAb.rawAtk`, `AttackSimple.excuse`, `AttackWave.excuse`, `AttackVolcano.excuse`, `AttackBlast.excuse`, and `EUnit/EEnemy.getDamage` mini-wave/mini-surge branches. `DamageCalculator` now exposes `rawAttackDamage` / `bcuProjectileBaseDamage`; wave, surge, and blast runtime containers store that raw base instead of the first direct target's `finalDamage`. Mini-wave/mini-surge and blast falloff are applied once to the projectile raw base before target-specific damage resolution.
- W2 resistance runtime: BCU references used: `EEnemy.getResistValue`, `EUnit.getResistValue`, `Data.SUPER_SAGE_RESIST`, `Data.SUPER_SAGE_RESIST_TYPE`, and `Data.SUPER_SAGE_HUNTER_RESIST`. `BcuResistRuntime` now returns `implemented: true` for supported proc resistance fields and centralizes duration, distance, and percent reductions used by `BcuProcImmunityPatch`.
- ZIP/effect evidence: `check-effect-bundle-aliases` verifies every `BattleWaveEffectLoader` alias against `public/assets/bundles/effect/wave.zip` and required status/kbeff entries against `status-effects.zip` and `kbeff.zip`. `check-effect-coordinate-traces` verifies deterministic effect trace fields `effectKey`, `phase`, `worldX`, `screenOffsetX`, `bcuSmokeYOffset`, `layer`, `scaleMode`, `renderFlipX`, and `bcuReference`.
