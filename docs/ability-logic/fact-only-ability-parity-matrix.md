...
| 地中移動 | `P_BURROW`; enemy 43/44 | movement/capture state; disappear and reappear by distance/count | `parseEnemyProc` | no burrow movement state found | raw zombie movement assets not fully mapped | `fact-partial` |
...
- W0 proof harness: added deterministic checks `scripts/check-bcu-parser-indexes.mjs`, `scripts/check-projectile-damage-parity.mjs`, `scripts/check-proc-immunity-resistance-parity.mjs`, `scripts/check-effect-bundle-aliases.mjs`, `scripts/check-effect-coordinate-traces.mjs`, and `scripts/check-debug-allocation-guards.mjs`. Follow-up pass replaced fake effect debug payload checks with real runtime spawn helper fixtures.
...
|---|---|---|---|---|---|---|---|---|
| Burrow | `DataEnemy.ints[43] -> BURROW.count`, `ints[44] / 4 -> BURROW.dis`; no normal `DataUnit` holder found | `Entity.update`, `update2`, `startBurrow`, `updateBurrow`, `touchable`; summon `anim_type == 3` can initialize burrow state | count decrements at start; `kbTime -2/-3/-4`; underground `bdist` decrements by actual movement; base proximity forces up animation | parser only: `BcuCombatModel.parseEnemyProc().burrow`; no `ProcResolver` catalog entry | actor animations `BURROW_DOWN/MOVE/UP`; no ZIP micro-effect proven | `ready-for-implementation` | runtime/capture/collision/renderability absent | `check-bcu-burrow-lifecycle-parity.mjs` |
| Castle/base guard | `StageBasis.activeGuard`; `ECastle.guard`; no actor proc holder | `StageBasis.update`, `StageBasis.checkGuard`, `Entity.postUpdate`, `ECastle.damaged/updateAnimation/guardBreak` | active guard blocks base HP damage entirely; break after boss condition clears | no proven JS `activeGuard` state or base damage gate | `EffAnim.A_E_GUARD`; `wave.zip` `enemy-wave-guard/image.png`, `imgcut.imgcut`, `model.mamodel`, `anim-none.maanim`, `anim-breaker.maanim` | `ready-for-implementation` | stage/base runtime state missing | `check-bcu-castle-guard-parity.mjs` |
| Spirit | `DataUnit.ints[110] -> SPIRIT.id`; `LineUp` builds spirit form | `StageBasis.act_spawn/update`, `spiritCooldown`, `summonerSummoned`, `spiritSummoned`, `EUnit.isSpirit` | cooldown 15; range 150; one spirit per living summoner; spirit rejects damage and self-kills after attack | parser/runtime missing | uses normal unit animation plus `A_IMUATK`; no separate spirit ZIP alias proven | `ready-for-implementation` | production/runtime owner not implemented locally | `check-bcu-spirit-lifecycle-parity.mjs` |
| Summon | `Proc.SUMMON` proc-object/custom attack holder; normal `DataUnit`/`DataEnemy` CSV holder not found | `AtkModelEntity.setProc/invokeLater`, `AtkModelUnit.summon`, `AtkModelEnemy.summon`, `Entity.setSummon`, `EntCont`, bond tree | random `dis..max_dis`; level/magnification scaled by `(100-resist)/100`; layer fallback; same_health and bond_hp flags | no JS holder parser or runtime hook | source-style summon/counter-surge paths in `wave.zip`; no stable summon runtime alias | `blocked` | JS custom/proc-object loader and actor creation contract missing | source loader check before runtime test |
...
| Enemy `IMUPOIATK` direct CSV holder | negative evidence: `DataEnemy` has `P_POIATK` 79/80 but no direct `IMUPOIATK` constructor column; `DataUnit` index 90 exists | `Entity.processProcs` still consumes target `getProc().IMUPOIATK` | full immunity blocks toxic, partial cuts toxic percent | JS models enemy `IMUPOIATK` as unconfirmed source | invalid effect on full immunity | `blocked` | holder may be custom/proc-object, not standard enemy CSV | custom holder source check |
| Zombie corpse / soulstrike / death surge | `DataEnemy` revive 45/46/47, death surge 89-92; `DataUnit` `AB_ZKILL` 52, `AB_CKILL` 98 | `Entity.ZombX`, `Entity.touchable`, `Entity.getTouch`, `AtkModelEntity.getDeathSurge` | revive HP `maxH * health / 100`; revive timer plus zombie effect length; death/mini-death surge random range | core JS revive/soulstrike/death trigger exists | `effect:soul` aliases; zombie revive visual alias not proven | `partial` | `REVIVE_SHOW_TIME`, corpse targetability, soulstrike cleanup, mini-death-surge split untested | `check-bcu-zombie-corpse-soulstrike-parity.mjs`, `check-bcu-death-surge-zombie-corpse-parity.mjs` |
...
- Wave guard / castle guard: 2026-06-04 source pass proved `StageBasis.activeGuard` and `ECastle.guard` as runtime owners. `A_E_GUARD` alias is bundled and loader-resolvable, but current JS has no completed stage/base guard state or base damage gate.
...
- Burrow and spirit: holder/runtime identity and owner methods are now source-backed enough for test-first implementation, but no JS lifecycle was implemented in this docs-only pass.
...
3. Do not expand death surge, counter surge, burrow, summon, spirit, or remaining external partial resistance sources until the blockers above are resolved in this matrix. `P_WARP` lifecycle is implemented to human-visual-review-needed evidence level; broader proc-vs-HP-KB cancellation ordering remains an audit item.
