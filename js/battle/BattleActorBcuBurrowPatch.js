...
import { clearBcuBurrow, canStartBcuBurrow, getBcuBurrowTouchMask, getRequiredBcuBurrowAnimationIds, hasBcuBurrowTemplateAnimations, hydrateBcuBurrowActorAnimations, isBcuBurrowNormallyTargetable, isBcuBurrowTargetableForEvent, startBcuBurrow, tickBcuBurrow } from './bcu-runtime/BcuBurrowLifecycleRuntime.js';

const ACTOR_PATCH_FLAG = Symbol.for('wanko-battle.actor-bcu-burrow.v4');
...
function reportBurrowAnimationUnavailable(scene, unitDef, template = null, reason = 'burrow-animation-definitions-missing') {
  scene?.pushEvent?.({ type: 'bcuBurrowAnimationUnavailable', slotId: unitDef?.slotId || template?.unitDef?.slotId || null, semanticKey: unitDef?.assetDef?.semanticKey || template?.assetDef?.semanticKey || null, reason, requiredAnimations: getRequiredBcuBurrowAnimationIds(), definedAnimations: definedBurrowAnimationIds(unitDef, template), bcuReference: 'TYPE7 burrow waits for BURROW_DOWN/BURROW_MOVE/BURROW_UP before entering underground state' });
}
function preloadBurrowAnimations(scene, unitDef, template = null, reason = 'burrow-animation-preload') {
  ...
  if (ids.length !== getRequiredBcuBurrowAnimationIds().length) { reportBurrowAnimationUnavailable(scene, unitDef, template); return null; }
...
function ensureActorBurrowAnimations(scene, actor) { const tpl = actorTemplate(scene, actor); if (tpl) hydrateBcuBurrowActorAnimations(actor, tpl); return tpl; }
...
export function installBattleActorBcuBurrowPatch() {
  ...
  if (proto && !proto[ACTOR_PATCH_FLAG]) {
    ...
    proto.getBcuTouchMask = function getBcuTouchMask() { return getBcuBurrowTouchMask(this); };
    proto.isBcuTargetableForEvent = function isBcuTargetableForEvent(event = null) { if (this.bcuBurrow?.active) return isBcuBurrowTargetableForEvent(this, event); if (typeof this.isTargetable === 'function') return this.isTargetable(); return this.hp > 0; };
    const originalIsTargetable = proto.isTargetable;
    proto.isTargetable = function isTargetableWithBcuBurrow() { if (this.bcuBurrow?.active && !isBcuBurrowNormallyTargetable(this)) return false; return originalIsTargetable.call(this); };
    const originalIsTouchable = proto.isTouchable;
    proto.isTouchable = function isTouchableWithBcuBurrow() { if (this.bcuBurrow?.active && !isBcuBurrowNormallyTargetable(this)) return false; return originalIsTouchable.call(this); };
    const originalTick = proto.tick;
    proto.tick = function tickWithBcuBurrow(dt) { if (this.bcuBurrow?.active) { tickBcuBurrow(this, dt, { scene: this.scene || globalThis.__APP__?.scene || null }); return; } return originalTick.call(this, dt); };
    const originalEnterDeadState = proto.enterDeadState;
    proto.enterDeadState = function enterDeadStateClearingBcuBurrow(nowMs = 0) { clearBcuBurrow(this, 'death'); return originalEnterDeadState.call(this, nowMs); };
  }
  ...
  if (sceneProto && !sceneProto[SCENE_PATCH_FLAG]) {
    ...
    if (typeof originalSpawnStageEnemy === 'function') {
      sceneProto.spawnStageEnemy = function spawnStageEnemyWithBurrowAnimations(unitDef, row) {
        ...
        if (result) for (const actor of (this.actors || []).slice(before)) ensureActorBurrowAnimations(this, actor);
        return result;
      };
    }
    const originalSpawnActor = sceneProto.spawnActor;
    if (typeof originalSpawnActor === 'function') {
      sceneProto.spawnActor = function spawnActorHydratingBurrowAnimations(unitDef, side, isPlayerProduced = false, options = {}) { const actor = originalSpawnActor.call(this, unitDef, side, isPlayerProduced, options); if (actor) ensureActorBurrowAnimations(this, actor); return actor; };
    }
    ...
    sceneProto.startActorAttack = function startActorAttackWithBcuBurrow(actor, target, targetType) {
      ...
      if (start.ok) { startBcuBurrow(actor, { scene: this }); return true; }
