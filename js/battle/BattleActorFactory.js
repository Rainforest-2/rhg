...
export const TEMPLATE_LOAD_LEVEL = { STATS:'stats', RENDER_CORE:'render-core', FULL_VISUAL:'full-visual' };
const LEVEL_RANK = { 'stats':1, 'render-core':2, 'full-visual':3 };
...
export class BattleActorFactory {
  ...
  async _preloadTemplateUpgrade(unitDef, level, options={}){
    ...
    const assetDef = tpl?.assetDef || this.findAsset(unitDef.assetId);
    ...
    if(LEVEL_RANK[level] >= LEVEL_RANK[TEMPLATE_LOAD_LEVEL.FULL_VISUAL]){
      const targets = new Set([unitDef.attackAnimId, unitDef.knockbackAnimId, ...(options.animIds||[])].filter(Boolean));
