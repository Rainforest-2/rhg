...
export class BattleAttackProfile {
  ...
  static getTimingParity(actor = null) {
    ...
    return {
      enabled,
      source: cfg?.source || (enabled ? 'bcu-getItv-parity-default' : 'disabled'),
      disableAttackPhaseMultiplier: enabled ? cfg?.disableAttackPhaseMultiplier !== false : false,
      disableMinAttackStartup: enabled ? cfg?.disableMinAttackStartup !== false : false,
      disableMinAttackAnim: enabled ? cfg?.disableMinAttackAnim !== false : false
    };
  }
  ...
  static fromActor(actor) {
    ...
    if (Array.isArray(stats.attackHits) && stats.attackHits.length) {
      ...
      return { source:'bcu-stats-attackHits', isRange:!!stats.isRange, events, animationMs, waitMs, maxEventAtMs, timingParity, bcuTiming, bcuAttackIntervalMs:bcuTiming.bcuAttackIntervalMs, bcuAttackIntervalFrames:bcuTiming.bcuAttackIntervalFrames };
    }
