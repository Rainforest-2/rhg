...
export class BattleAttackProfile {
  static fromActor(actor) {
    ...
    if (Array.isArray(stats.attackHits) && stats.attackHits.length) {
      ...
      const safeEvents = events.length ? events : [{ key: 'hit-0', hitIndex: 0, atMs: minStartup, damage: actor?.damage ?? 0, rangeStartPx: 0, rangeEndPx: actor?.detectionRangePx ?? 0, targetMode: 'single' }];
    ...
    return { source: 'actor-current-stats', isRange: actor?.attackType === 1, events: [{ key: 'hit-0', hitIndex: 0, atMs: effectiveStartupMs, damage: actor?.damage ?? 0, rangeStartPx: 0, rangeEndPx: actor?.detectionRangePx ?? 0, targetMode: 'single' }], animationMs: attackEndMs, waitMs: actor?.attackWaitMs ?? 0, maxEventAtMs: effectiveStartupMs };
  }
