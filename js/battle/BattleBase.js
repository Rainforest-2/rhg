import { BattleCastleResolver } from './BattleCastleResolver.js';
...
export class BattleBase {
  constructor({ id, side, label, x, y, posBcu = null, battleCoordinate = null, maxHp = 1000, collisionRadius = 80, attackable = true, visualAssetId = null, visualKind = 'simple-placeholder', layers = [], scale = 1.0, visualBottomToCurrentCenter = false, visualYOffsetPx = 0, combatBodyHalfWidthPx = null, combatBodyHeightPx = null, combatBodyYOffsetPx = 0, debug = {} }) {
    ...
    this.combatBodyBoxOverride = null;
    this.frontX = null;
    this.combatBodySource = 'constructor-default';
  }

  updateCombatBodyFromVisualBounds(bounds, scale = 1, side = this.side, options = {}) {
    const asset = options.asset || this.castleAsset || (bounds ? { visualBounds: bounds } : null);
    const geometry = BattleCastleResolver.applyToBase(this, {
      asset,
      scale,
      source: options.source || 'BattleBase.updateCombatBodyFromVisualBounds',
      fallbackWidth: bounds?.width || this.combatBodyHalfWidthPx * 2 || this.collisionRadius * 2,
      fallbackHeight: bounds?.height || this.combatBodyHeightPx || this.collisionRadius * 2
    });
    this.debug = {
      ...(this.debug || {}),
      castleGeometry: geometry,
      castleBodySource: geometry?.bodySource || null,
      castleFrontX: geometry?.frontX ?? null,
      castleVisualWidth: geometry?.visualBounds?.width ?? null,
      castleVisualHeight: geometry?.visualBounds?.height ?? null
    };
    return { halfWidth: this.combatBodyHalfWidthPx, height: this.combatBodyHeightPx, side, geometry };
  }
  ...
  getFrontX() {
    if (Number.isFinite(this.frontX)) return this.frontX;
  ...
  getCombatBodyBox() {
    if (this.combatBodyBoxOverride) return this.combatBodyBoxOverride;
    ...
    return { left: this.x - halfW, right: this.x + halfW, top: bottom - height, bottom, centerX: this.x, centerY: bottom - height * 0.5, halfWidth: halfW, width: halfW * 2, height, source: this.combatBodySource || 'base-radius-body' };
  }
