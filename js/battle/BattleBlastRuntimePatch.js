...
function attackBlast(scene, item, level) {
  ...
  scene.lastBlastAttackDebug = { id: item.id, level, targetCount: targets.length, appliedCount: applied, damage, ranges, source: 'BattleBlastRuntimePatch.attackBlast' };
...
function enqueue(scene, item) {
  ...
  scene.lastBlastCreatedDebug = { id: item.id, actor: item.attacker?.instanceId || item.attacker?.label || null, pos: item.pos, effectKey: item.effectKey, source: 'BattleBlastRuntimePatch.enqueue' };
  scene.pushEvent?.({ type: 'bcuBlastCreated', ...scene.lastBlastCreatedDebug });
}
...
function enqueueFromResult(scene, attacker, target, targetType, event, calc, result, meta = {}) {
  ...
  scene.lastBlastResolveDebug = {
    source: 'BattleBlastRuntimePatch.enqueueFromResult',
    targetType,
