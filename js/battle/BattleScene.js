...
export class BattleScene {
  constructor(uiLog) { this.loader = new BcuAssetLoader(); this.statsLoader = new BattleStatsLoader(); this.log = uiLog || (() => {}); this.mode = 'battle'; this.groundY = BATTLE_CONFIG.groundY; this.actors = []; this.castle = { ...BATTLE_CONFIG.castle, layers: [] }; this.stage = { ...BATTLE_CONFIG.stage, backgroundImageAsset: null, backgroundLoadFailed: false }; this.loadFailed = false; this.failureReason = ''; }
  ...
  async loadActor(actorDef) {
    ...
    const animationDefs = [actorDef.idleAnimId, actorDef.moveAnimId, actorDef.attackAnimId];
    const animations = {};
    for (const animId of animationDefs) {
    ...
    if (!animations[actorDef.attackAnimId]) this.log('warn', `attack animation fallback to anim00 for ${actorDef.assetId}`);
    ...
    actor.moveAnimId = actorDef.moveAnimId;
    actor.attackAnimId = actorDef.attackAnimId;
    return actor;
  }

  async init() {
    try {
      this.loadFailed = false; this.failureReason = '';
      try { this.stage.backgroundImageAsset = await loadImage(this.stage.backgroundImage); this.stage.backgroundLoadFailed = false; }
      catch (e) { this.stage.backgroundImageAsset = null; this.stage.backgroundLoadFailed = true; this.log('warn', `battle background fallback: ${e instanceof Error ? e.message : String(e)}`); }
  ...
  tick(dt) {
    ...
    const dog = this.actors.find((a) => a.side === 'dog-player'); const cat = this.actors.find((a) => a.side === 'cat-enemy');
    if (!dog || !cat) return;
    const alive = this.actors.filter((a) => a.isAlive());
    for (const actor of alive) actor.tick(dt);
    if (!dog.isAlive() || !cat.isAlive()) return;

    const dtSec = dt / 1000;
    const distance = Math.abs(dog.x - cat.x);
    const stopDistance = Math.min(dog.detectionRangePx, cat.detectionRangePx);

    if (distance > stopDistance) {
      dog.setState('move'); cat.setState('move');
      dog.setAnimation(dog.moveAnimId); cat.setAnimation(cat.moveAnimId);
      dog.x += dog.direction * dog.moveSpeed * dtSec;
      cat.x += cat.direction * cat.moveSpeed * dtSec;
      return;
    }
