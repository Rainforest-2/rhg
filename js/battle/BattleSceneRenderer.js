...
export class BattleSceneRenderer {
  projectX(scene, worldX){ return scene?.camera ? scene.camera.worldToScreenX(worldX) : worldX; }
  ...
  drawBase(c,base,groundY,showParts){ if(base.visualKind==='bcu-enemy-castle'&&base.castleAsset?.image){this.drawBcuEnemyCastle(c,base);} else if(base.visualKind==='castle-composite'&&base.layers?.length){ const visualYOffset = this.getBaseVisualYOffset(base); for(const layer of base.layers){const s=base.scale||1;const x=this.projectX(this._scene,base.x+(layer.offsetX||0)*s)-layer.image.width*0.5*s;const y=base.y+visualYOffset+(layer.offsetY||0)*s-layer.image.height*s;c.drawImage(layer.image,x,y,layer.image.width*s,layer.image.height*s);} } else { const pw=BATTLE_CONFIG.visualLayout?.catBasePlaceholder?.width??100; const ph=BATTLE_CONFIG.visualLayout?.catBasePlaceholder?.height??80; const ly=BATTLE_CONFIG.visualLayout?.catBasePlaceholder?.labelYOffset??8; c.fillStyle='#374151'; const sx=this.projectX(this._scene,base.x); c.fillRect(sx-pw*0.5, base.y-ph, pw, ph); c.fillStyle='#e5e7ebcc'; c.fillText('CAT BASE TEMP', sx-pw*0.44, base.y-ph-ly);} if(showParts) this.drawBaseDebug(c,base); }

  drawBcuEnemyCastle(c,base){const a=base.castleAsset;const crop=a?.crop;if(!a?.image||!crop)return;const s=Number.isFinite(base.scale)?base.scale:1;const sx=this.projectX(this._scene,base.x);const drawW=crop.w*s,drawH=crop.h*s;const drawX=sx-drawW*0.5;const drawY=base.y-drawH+(Number.isFinite(base.visualYOffsetPx)?base.visualYOffsetPx:0);c.drawImage(a.image,crop.x,crop.y,crop.w,crop.h,drawX,drawY,drawW,drawH);}
