...
export class BattleSceneRenderer {
  render(previewRenderer, scene, showParts = false) {
    ...
    if (showParts) { this.drawCombatDebug(c, scene); for (const actor of actorsForRender) this.drawActorDebug(c, actor, scene?.battleState || 'running'); this.drawEventLog(c, scene?.debugEvents || []); }
  ...
  drawCombatDebug(c, scene){const actors=(scene?.actors||[]).filter((a)=>a?.isAlive?.());const bases=scene?.bases||[];c.save();c.lineWidth=2;c.font='11px ui-monospace, monospace';for(const actor of actors){const box=this.getDebugCombatBodyBox(actor);if(!box)continue;const dir=Number.isFinite(actor.direction)?actor.direction:1;const debug=BattleBodyResolver.getAttackRangeDebug(actor);const frontX=debug.frontX;const isPoint=!!box.isCombatPoint;const attackLineX=debug.attackLineX;c.strokeStyle=actor.side==='dog-player'?'#f97316':'#38bdf8';c.strokeRect(box.left,box.top,box.right-box.left,box.bottom-box.top);c.strokeStyle='#e5e7eb';c.beginPath();c.moveTo(actor.x,box.top-8);c.lineTo(actor.x,box.bottom+8);c.stroke();c.strokeStyle='#f8fafc';c.lineWidth=3;c.beginPath();c.moveTo(frontX,box.top-8);c.lineTo(frontX,box.bottom+8);c.stroke();c.lineWidth=2;if(!isPoint){const backX=Number.isFinite(box.backX)?box.backX:(dir<0?box.right:box.left);c.globalAlpha=0.5;c.strokeStyle=actor.side==='dog-player'?'#f97316':'#38bdf8';c.beginPath();c.moveTo(backX,box.top-8);c.lineTo(backX,box.bottom+8);c.stroke();c.globalAlpha=1;}c.strokeStyle='#fde047';c.beginPath();c.moveTo(attackLineX,box.top-12);c.lineTo(attackLineX,box.bottom+12);c.stroke();c.beginPath();c.moveTo(frontX,box.centerY);c.lineTo(attackLineX,box.centerY);c.stroke();const profile=actor.getAttackProfile?.()||BattleAttackProfile.ensure(actor);const events=Array.isArray(profile?.events)?profile.events:[];for(const event of events){const line=BattleAttackResolver.getAttackDebugLine(actor,event);const kind=event?.attackKind||event?.raw?.attackKind||'normal';const color=kind==='omni'?'#e879f9':(kind==='ld'?'#fb923c':'#facc15');c.save();if(kind==='normal')c.setLineDash([4,4]);else c.setLineDash([]);c.strokeStyle=color;c.beginPath();c.moveTo(line.left,line.centerY);c.lineTo(line.right,line.centerY);c.stroke();c.beginPath();c.moveTo(line.left,line.top);c.lineTo(line.left,line.bottom);c.moveTo(line.right,line.top);c.lineTo(line.right,line.bottom);c.stroke();c.beginPath();c.moveTo(line.frontX,line.centerY-6);c.lineTo(line.frontX,line.centerY+6);c.stroke();c.restore();}const eventText=events.slice(0,3).map((e)=>`${e.hitIndex}:${Math.round(e.atMs)}ms/${Math.round(e.damage||0)}/${e.attackKind||e.raw?.attackKind||'normal'}`).join(' ');const statSrc=actor.statSource?`${actor.statSource.statsType||'-'}:${Number.isFinite(actor.statSource.row)?actor.statSource.row:'-'}`:'-';const combatX=BattleBodyResolver.getActorCombatPositionX?.(actor);const cpos=Number.isFinite(combatX)?Math.round(combatX):'-';const coff=Math.round(actor.combatPositionOffsetPx||0);const auto=Math.round(actor.autoCombatPositionOffsetWorldPx||0);const cand=actor.autoCombatPositionCandidateCount??'-';const rej=actor.autoCombatPositionRejectedCount??'-';const csrc=actor.combatPositionSource||actor.autoCombatPositionSource||actor.combatBodyFrontSource||'-';c.fillStyle='#f8fafc';c.fillText(`pos:${cpos} body:${Math.round(box.left)}..${Math.round(box.right)} range:${Math.round(actor.detectionRangePx||0)} man:${coff} auto:${auto} src:${csrc} cand:${cand} rej:${rej} stat:${statSrc} atkEvents:${events.length} mode:${profile?.isRange?'range':'single'} ${eventText}`,box.left,box.top-4);}for(const base of bases){const box=this.getDebugCombatBodyBox(base);if(!box)continue;c.strokeStyle='#a78bfa';c.strokeRect(box.left,box.top,box.right-box.left,box.bottom-box.top);c.beginPath();c.moveTo(box.centerX,box.top);c.lineTo(box.centerX,box.bottom);c.stroke();c.fillStyle='#f8fafc';c.fillText(`base body ${Math.round(box.left)}..${Math.round(box.right)}`,box.left,box.top-4);}for(const effect of scene?.effects||[]){if(!effect||effect.finished)continue;c.strokeStyle='#ef4444';c.beginPath();c.moveTo(effect.x-8,effect.y);c.lineTo(effect.x+8,effect.y);c.moveTo(effect.x,effect.y-8);c.lineTo(effect.x,effect.y+8);c.stroke();}c.restore();}
  ...
  drawActorDebug(c, actor, battleState) { const src = actor.rawStats?.source || {}; const d = actor.debugDistance || {}; const lines = [`${actor.instanceId||'-'} slot:${actor.slotId||'-'} state:${actor.state} battle:${battleState}`,`anim:${actor.currentAnimId} role:${actor.activeAnimRole} target:${actor.currentTargetLabel||'-'} type:${actor.currentTargetType||'-'}`,`bodyDistance:${(d.bodyDistance ?? 0).toFixed(1)} moveSpeed:${(actor.moveSpeed||0).toFixed(1)} sideBlocking:disabled spacing:spawn-only`,`kbElapsed:${(actor.knockbackPositionElapsedMs||0).toFixed(1)} kbDur:${(actor.knockbackPositionDurationMs||0).toFixed(1)} from:${(actor.knockbackFromX||0).toFixed(1)} to:${(actor.knockbackToX||0).toFixed(1)} pendingDmg:${actor.pendingDamage||0} kbReason:${actor.knockbackReason||'-'} death:${actor.state==='dead'?`${Math.round(actor.deathElapsedMs||0)}/${actor.removeAfterMs||0}`:'-'}`,`mappingStatus:${src.mappingStatus || '-'} fallback:${(src.fallbackFields||[]).join('|')||'-'}`]; c.fillStyle = '#0008'; c.fillRect(actor.x - 170, actor.y - 190, 560, 62); c.fillStyle = '#f8fafc'; c.font = '12px ui-monospace, monospace'; lines.forEach((line, i) => c.fillText(line, actor.x - 164, actor.y - 176 + i * 14)); }
