...
export class BattleSceneRenderer {
  render(previewRenderer, scene, debugOptions = false) {
    ...
    c.clearRect(0, 0, w, h);
    if (scene?.stage?.background?.image && scene?.stage?.background?.crop) { if (BATTLE_CONFIG.stage.backgroundMode === 'bcu-stage0') this.drawBackgroundBcuStage0(c, scene, scene.stage.background, w, h); else this.drawBackgroundCropCover(c, scene.stage.background, w, h); } else this.drawFallbackBackground(c, w, h, groundY);
    ...
    this.drawBases(c, scene?.bases || [], groundY, debug.showBounds);
    ...
    if (debug.showBounds) { this.drawCombatDebug(c, scene, debug); for (const actor of actorsForRender) this.drawActorDebug(c, actor, scene?.battleState || 'running'); this.drawEventLog(c, scene?.debugEvents || []); }
  ...
  drawBackgroundBcuStage0(c,scene,bg,w,h){const colors=bg.colors;const crop=bg.crop;const image=bg.image;const coord=scene?.coordinate;if(!image||!crop||!colors||!coord){this.drawBackgroundCropCover(c,bg,w,h);return;}const scale=coord.battleSiz||1;const fw=crop.w*scale;const fh=crop.h*scale;const tileStartX=(coord.cameraX||0)+(coord.offP||200)*scale-fw;const bgBottomY=scene?.groundY??h;const dy=bgBottomY-fh;this._lastBgDebug={scale,tileStartX,bgBottomY,cropW:crop.w,cropH:crop.h,bgBottomYSource:'provisional-browser-groundY'};this.drawVerticalGradient(c,0,0,w,h,colors.skyTop,colors.skyBottom);for(let x=tileStartX;x<w+fw;x+=fw){c.drawImage(image,crop.x,crop.y,crop.w,crop.h,x,dy,fw,fh);}if(bg.upperCrop){const up=bg.upperCrop;const uh=up.h*scale;const uy=Math.max(0,dy-uh);for(let x=tileStartX;x<w+fw;x+=fw){c.drawImage(image,up.x,up.y,up.w,up.h,x,uy,up.w*scale,uh);}}if(bgBottomY<h)this.drawVerticalGradient(c,0,bgBottomY,w,h-bgBottomY,colors.groundTop,colors.groundBottom);}
  ...
  drawCombatDebug(c, scene){const coord=scene?.coordinate;const cd=coord?.describe?.()||{};c.save();c.fillStyle='#0008';c.fillRect(860,14,400,170);c.fillStyle='#f8fafc';c.font='12px ui-monospace';c.fillText(`v:${BATTLE_CONFIG.version} mode:${cd.projectionMode||'-'}`,868,30);c.fillText(`ratio:${cd.ratioPPerBcu} off:${cd.offP} stage:${cd.stageLenBcu}(${cd.stageLenSource||'-'})`,868,46);c.fillText(`worldP:${(cd.worldWidthP??0).toFixed?.(2)||cd.worldWidthP} siz:${(cd.battleSiz??0).toFixed?.(4)||cd.battleSiz} camX:${cd.cameraX}`,868,62);c.fillText(`pxPerBcu:${(cd.pxPerBcu??0).toFixed?.(4)||cd.pxPerBcu} L:${Math.round(cd.leftBasePosBcu||0)}->${Math.round(cd.leftBaseScreenX||0)} R:${Math.round(cd.rightBasePosBcu||0)}->${Math.round(cd.rightBaseScreenX||0)}`,868,78);c.fillText(`h:${cd.horizontalProjectionSource||'-'} v:${cd.verticalProjectionSource||'-'}`,868,94);c.fillText(`actorScale:${cd.actorVisualScaleSource||'-'} baseScale:${cd.baseVisualScaleSource||'-'}`,868,110);c.fillText(`bgVertical:${cd.backgroundVerticalSource||'-'} spawn:${cd.spawnOffsetSource||'-'} speed:${cd.speedSource||'-'}`,868,126);c.fillText(`src:${cd.source||'-'}`,868,142);if(this._lastBgDebug){c.fillText(`bg siz:${this._lastBgDebug.scale.toFixed(4)} tileStartX:${Math.round(this._lastBgDebug.tileStartX)} crop:${this._lastBgDebug.cropW}x${this._lastBgDebug.cropH}`,868,158);}c.restore();const actors=(scene?.actors||[]).filter((a)=>a?.isAlive?.());const bases=scene?.bases||[];c.save();c.lineWidth=2;c.font='11px ui-monospace, monospace';for(const actor of actors){const box=this.getDebugCombatBodyBox(actor);if(!box)continue;const debug=BattleBodyResolver.getAttackRangeDebug(actor);const actorX=Number.isFinite(actor?.x)?actor.x:0;const posX=Number.isFinite(box.combatPositionX)?box.combatPositionX:debug.frontX;const edgeOffset=Number.isFinite(actor?.resolvedCombatEdgeWorldOffsetPx)?actor.resolvedCombatEdgeWorldOffsetPx:0;const edgeX=actorX+edgeOffset;const sideColor=actor.side==='dog-player'?'#f97316':'#38bdf8';c.strokeStyle=sideColor;c.globalAlpha=0.35;c.strokeRect(box.left,box.top,box.right-box.left,box.bottom-box.top);c.globalAlpha=1;c.strokeStyle='#9ca3af';c.lineWidth=1;c.beginPath();c.moveTo(actorX,box.top-12);c.lineTo(actorX,box.bottom+12);c.stroke();c.strokeStyle='#f8fafc';c.lineWidth=3;c.beginPath();c.moveTo(posX,box.top-12);c.lineTo(posX,box.bottom+12);c.stroke();c.strokeStyle='#86efac';c.lineWidth=2;c.beginPath();c.moveTo(edgeX,box.top-8);c.lineTo(edgeX,box.bottom+8);c.stroke();const profile=actor.getAttackProfile?.()||BattleAttackProfile.ensure(actor);for(const event of (profile?.events||[])){const line=BattleAttackResolver.getAttackDebugLine(actor,event);const kind=event?.attackKind||event?.raw?.attackKind||'normal';const color=kind==='omni'?'#a855f7':(kind==='ld'?'#fb923c':'#facc15');c.save();if(kind==='normal')c.setLineDash([4,4]);c.strokeStyle=color;c.beginPath();c.moveTo(line.left,line.centerY);c.lineTo(line.right,line.centerY);c.stroke();c.restore();c.fillStyle=color;c.fillText(`atk:${kind} bcu:${Math.round(line.leftBcu||0)}..${Math.round(line.rightBcu||0)} px:${Math.round(line.left)}..${Math.round(line.right)}`,line.left,line.top-2);}const cand=actor.resolvedCombatEdgeCandidateCount??0;const rej=actor.resolvedCombatEdgeRejectedCount??0;const edgeSrc=actor.resolvedCombatEdgeSource||'-';const inset=actor.combatEdgeInsetPx||0;const statSrc=actor.statSource?`${actor.statSource.statsType||'-'}:${Number.isFinite(actor.statSource.row)?actor.statSource.row:'-'}`:'-';c.fillStyle='#f8fafc';c.fillText(`screenX:${Math.round(posX)} posBcu:${Math.round(actor.posBcu||0)} rangeBcu:${Math.round(debug.rangeBcu||0)} widthBcu:${Math.round(debug.widthBcu||0)} edge:${edgeSrc} inset:${inset} cand:${cand}/${rej} body:${Math.round(box.left)}..${Math.round(box.right)} stat:${statSrc} atkEvents:${(profile?.events||[]).length}`,box.left,box.top-4);}for(const base of bases){const box=this.getDebugCombatBodyBox(base);if(!box)continue;c.strokeStyle='#a78bfa';c.strokeRect(box.left,box.top,box.right-box.left,box.bottom-box.top);}c.restore();}
  drawHud(c, scene, debug) { const dog = scene?.actors?.find((a) => a.side === 'dog-player'); const cat = scene?.actors?.find((a) => a.side === 'cat-enemy'); const dogBase=scene?.bases?.find((b)=>b.side==='dog-player'); const catBase=scene?.bases?.find((b)=>b.side==='cat-enemy'); const aliveDogs=(scene?.actors||[]).filter(a=>a.isAlive()&&a.side==='dog-player').length; const aliveCats=(scene?.actors||[]).filter(a=>a.isAlive()&&a.side==='cat-enemy').length; c.fillStyle = '#0008'; c.fillRect(14, 14, 860, 170); c.fillStyle = '#f8fafc'; c.font = '20px ui-sans-serif'; const version = BATTLE_CONFIG.version || '0.0.0'; c.fillText(`Wanko Battle v${version}`, 24, 40); c.font = '14px ui-monospace, monospace'; c.fillText(`dogBase HP:${dogBase?.hp ?? '-'} catBase HP:${catBase?.hp ?? '-'}`,24,62); c.fillText(`dog HP/state: ${dog?.hp ?? '-'} / ${dog?.state ?? '-'}`, 24, 84); c.fillText(`cat HP/state: ${cat?.hp ?? '-'} / ${cat?.state ?? '-'}`, 24, 104); c.fillText(`money:${Math.floor(scene?.economy?.money||0)}/${scene?.economy?.maxMoney||0} dogs:${aliveDogs} cats:${aliveCats}`,24,124); c.fillText(`battleState:${scene?.battleState || '-'} debug:${debug?.showBounds?'ON':'OFF'} effects:${(scene?.effects||[]).length}`,24,144); }
  ...
  drawActorDebug(c, actor, battleState) { const src = actor.rawStats?.source || {}; const d = actor.debugDistance || {}; const lifeState = actor.isAlive?.() ? 'alive' : (actor.isFinalKnockback?.() ? 'finalKB' : actor.state); const deadAt = Number.isFinite(actor.deadAtMs) ? Math.round(actor.deadAtMs) : '-'; const lines = [`${actor.instanceId||'-'} slot:${actor.slotId||'-'} state:${actor.state} battle:${battleState} life:${lifeState}`,`anim:${actor.currentAnimId} role:${actor.activeAnimRole} target:${actor.currentTargetLabel||'-'} type:${actor.currentTargetType||'-'}`,`bodyDistance:${(d.bodyDistance ?? 0).toFixed(1)} moveSpeed:${(actor.moveSpeed||0).toFixed(1)} sideBlocking:disabled spacing:spawn-only`,`kbElapsed:${(actor.knockbackPositionElapsedMs||0).toFixed(1)} kbDur:${(actor.knockbackPositionDurationMs||0).toFixed(1)} from:${(actor.knockbackFromX||0).toFixed(1)} to:${(actor.knockbackToX||0).toFixed(1)} pendingDmg:${actor.pendingDamage||0} kbReason:${actor.knockbackReason||'-'} deathAfterKB:${actor.deathAfterKnockback?'Y':'-'} deadAt:${deadAt}`,`mappingStatus:${src.mappingStatus || '-'} fallback:${(src.fallbackFields||[]).join('|')||'-'}`]; c.fillStyle = '#0008'; c.fillRect(actor.x - 170, actor.y - 190, 560, 62); c.fillStyle = '#f8fafc'; c.font = '12px ui-monospace, monospace'; lines.forEach((line, i) => c.fillText(line, actor.x - 164, actor.y - 176 + i * 14)); }
