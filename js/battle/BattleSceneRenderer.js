...
export class BattleSceneRenderer {
  ...
  drawActorDebug(c, actor, battleState) { const d = actor.debugDistance || {}; const lifeState = actor.isAlive?.() ? 'alive' : (actor.isFinalKnockback?.() ? 'finalKB' : actor.state); const frontScreenOffset = Number.isFinite(actor.visualRenderOffsetDebug?.frontScreenOffset) ? Math.round(actor.visualRenderOffsetDebug.frontScreenOffset) : '-'; const lines = [`${actor.instanceId||'-'} st:${actor.state} life:${lifeState} b:${battleState}`,`posBcu:- screenX:${Math.round(actor.x||0)} renderOff:${Math.round(actor.visualRenderOffsetWorldPx||0)} src:${actor.visualRenderOffsetSource||'-'}`,`frontOff:${frontScreenOffset} render offset does not affect combat`,`dist:${(d.bodyDistance ?? 0).toFixed(1)} spd:${(actor.moveSpeed||0).toFixed(1)} rangePx:${Math.round(actor.detectionRangePx||0)}`,`rangeBcu:- widthBcu:- combat:white visual:green`]; c.fillStyle = '#0008'; c.fillRect(actor.x - 170, actor.y - 190, 560, 62); c.fillStyle = '#f8fafc'; c.font = '12px ui-monospace, monospace'; lines.forEach((line, i) => c.fillText(line, actor.x - 164, actor.y - 176 + i * 14)); }
