export class BattleSceneRenderer {
  render(previewRenderer, scene) {
    ...
    c.fillStyle = '#c9b78f'; c.fillRect(0, groundY, w, h - groundY); c.strokeStyle = '#7c6643'; c.lineWidth = 3; c.beginPath(); c.moveTo(0, groundY + 0.5); c.lineTo(w, groundY + 0.5); c.stroke();

    this.drawCastle(c, scene?.castle, groundY);
    for (const actor of (scene?.actors || [])) { this.drawActor(c, actor); this.drawActorDebug(c, actor); }

    c.fillStyle = '#0008'; c.fillRect(14, 14, 520, 104); c.fillStyle = '#f8fafc'; c.font = '20px ui-sans-serif, system-ui';
    c.fillText('Wanko Battle v0.1', 24, 40); c.font = '14px ui-monospace, monospace';
    c.fillText('dog-player starts right -> moves left', 24, 62);
    c.fillText('cat-enemy starts left -> moves right', 24, 82);
    c.fillText('mode:battle', 24, 100);
  ...
  drawActorDebug(c, actor) {
    ...
    const lines = [
      ...
      `x:${actor.x.toFixed(1)} facing:${actor.facing} dir:${actor.direction} state:${actor.state}`,
      `hp:${actor.hp}/${actor.maxHp} dmg:${actor.damage}`,
      `speedRaw:${actor.rawStats?.speed ?? '-'} moveSpeed:${actor.moveSpeed.toFixed(1)}px/s`,
      `detectionRangeRaw:${actor.rawStats?.detectionRange ?? '-'} detectionRangePx:${actor.detectionRange.toFixed(1)}`,
    ...
    c.fillStyle = '#0009'; c.fillRect(actor.x - 150, actor.y - 175, 380, 108); c.fillStyle = '#f8fafc'; c.font = '12px ui-monospace, monospace'; lines.forEach((line, i) => c.fillText(line, actor.x - 144, actor.y - 160 + i * 14));
  }
  ...
  drawActor(c, actor) {
    ...
    if (actor.visualFacing < 0) c.scale(-1, 1);
