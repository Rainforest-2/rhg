...
export class BattleSceneRenderer {
  ...
  getDebugCombatBodyBox(entity){if(typeof entity?.getCombatBodyBox==='function')return entity.getCombatBodyBox();const radius=entity?.collisionRadius||0;const bottom=Number.isFinite(entity?.y)?entity.y:0;const height=radius*2;return{left:entity.x-radius,right:entity.x+radius,top:bottom-height,bottom,centerX:entity.x,centerY:bottom-height*0.5,halfWidth:radius,height};}

  drawCombatDebug(c, scene){const actors=(scene?.actors||[]).filter((a)=>a?.isAlive?.());const bases=scene?.bases||[];c.save();c.lineWidth=2;c.font='11px ui-monospace, monospace';for(const actor of actors){const box=this.getDebugCombatBodyBox(actor);if(!box)continue;const dir=Number.isFinite(actor.direction)?actor.direction:1;const frontX=Number.isFinite(box.frontX)?box.frontX:(dir<0?box.left:box.right);const backX=Number.isFinite(box.backX)?box.backX:(dir<0?box.right:box.left);const attackLineX=frontX+dir*(actor.detectionRangePx||0);c.strokeStyle=actor.side==='dog-player'?'#f97316':'#38bdf8';c.strokeRect(box.left,box.top,box.right-box.left,box.bottom-box.top);c.strokeStyle='#e5e7eb';c.beginPath();c.moveTo(actor.x,box.top-8);c.lineTo(actor.x,box.bottom+8);c.stroke();c.strokeStyle=actor.side==='dog-player'?'#f97316':'#38bdf8';c.lineWidth=3;c.beginPath();c.moveTo(frontX,box.top-8);c.lineTo(frontX,box.bottom+8);c.stroke();c.lineWidth=2;c.globalAlpha=0.5;c.beginPath();c.moveTo(backX,box.top-8);c.lineTo(backX,box.bottom+8);c.stroke();c.globalAlpha=1;c.strokeStyle='#fde047';c.beginPath();c.moveTo(attackLineX,box.top-12);c.lineTo(attackLineX,box.bottom+12);c.stroke();c.beginPath();c.moveTo(frontX,box.centerY);c.lineTo(attackLineX,box.centerY);c.stroke();c.fillStyle='#f8fafc';c.fillText(`front:${Math.round(frontX)} body:${Math.round(box.left)}..${Math.round(box.right)} range:${Math.round(actor.detectionRangePx||0)} src:${actor.combatBodyFrontSource||'-'}`,box.left,box.top-4);}for(const base of bases){const box=this.getDebugCombatBodyBox(base);if(!box)continue;c.strokeStyle='#a78bfa';c.strokeRect(box.left,box.top,box.right-box.left,box.bottom-box.top);c.beginPath();c.moveTo(box.centerX,box.top);c.lineTo(box.centerX,box.bottom);c.stroke();c.fillStyle='#f8fafc';c.fillText(`base body ${Math.round(box.left)}..${Math.round(box.right)}`,box.left,box.top-4);}for(const effect of scene?.effects||[]){if(!effect||effect.finished)continue;c.strokeStyle='#ef4444';c.beginPath();c.moveTo(effect.x-8,effect.y);c.lineTo(effect.x+8,effect.y);c.moveTo(effect.x,effect.y-8);c.lineTo(effect.x,effect.y+8);c.stroke();}c.restore();}
