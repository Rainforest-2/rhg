...
export class BattleSceneRenderer {
  ...
  drawCombatDebug(c, scene){const actors=(scene?.actors||[]).filter((a)=>a?.isAlive?.());const bases=scene?.bases||[];c.save();c.lineWidth=2;c.font='11px ui-monospace, monospace';for(const actor of actors){const box=this.getDebugCombatBodyBox(actor);if(!box)continue;const debug=BattleBodyResolver.getAttackRangeDebug(actor);const actorX=Number.isFinite(actor?.x)?actor.x:0;const posX=Number.isFinite(box.combatPositionX)?box.combatPositionX:debug.frontX;const edgeOffset=Number.isFinite(actor?.resolvedCombatEdgeWorldOffsetPx)?actor.resolvedCombatEdgeWorldOffsetPx:0;const edgeX=actorX+edgeOffset;const renderOriginX=actorX+(Number.isFinite(actor?.visualRenderOffsetWorldPx)?actor.visualRenderOffsetWorldPx:0);const crowdVisualX=renderOriginX+(Number.isFinite(actor?.visualCrowdFanoutPx)?actor.visualCrowdFanoutPx:0);const sideColor=actor.side==='dog-player'?'#f97316':'#38bdf8';c.strokeStyle=sideColor;c.globalAlpha=0.35;c.strokeRect(box.left,box.top,box.right-box.left,box.bottom-box.top);c.globalAlpha=1;c.strokeStyle='#9ca3af';c.lineWidth=1;c.beginPath();c.moveTo(actorX,box.top-12);c.lineTo(actorX,box.bottom+12);c.stroke();c.strokeStyle='#f8fafc';c.lineWidth=3;c.beginPath();c.moveTo(posX,box.top-12);c.lineTo(posX,box.bottom+12);c.stroke();c.strokeStyle='#86efac';c.lineWidth=2;c.beginPath();c.moveTo(edgeX,box.top-8);c.lineTo(edgeX,box.bottom+8);c.stroke();c.strokeStyle='#ff00ff';c.lineWidth=2;c.beginPath();c.moveTo(renderOriginX,box.top-6);c.lineTo(renderOriginX,box.bottom+6);c.stroke();c.strokeStyle='#22d3ee';c.lineWidth=2;c.beginPath();c.moveTo(crowdVisualX,box.top-4);c.lineTo(crowdVisualX,box.bottom+4);c.stroke();const profile=actor.getAttackProfile?.()||BattleAttackProfile.ensure(actor);for(const event of (profile?.events||[])){const line=BattleAttackResolver.getAttackDebugLine(actor,event);if(!line||!Number.isFinite(line.left)||!Number.isFinite(line.right)||!Number.isFinite(line.centerY)||!Number.isFinite(line.top))continue;const kind=event?.attackKind||event?.raw?.attackKind||'normal';const color=kind==='omni'?'#a855f7':(kind==='ld'?'#fb923c':'#facc15');const y=Number.isFinite(line.centerY)?line.centerY:(box.centerY??actor.y??0);const labelY=Number.isFinite(line.top)?(line.top-2):(y-16);if(!Number.isFinite(y)||!Number.isFinite(labelY))continue;c.save();if(kind==='normal')c.setLineDash([4,4]);c.strokeStyle=color;c.beginPath();c.moveTo(line.left,y);c.lineTo(line.right,y);c.stroke();c.restore();c.fillStyle=color;const labelX=Math.min(line.left,line.right);const mode=line.mode||'unknown';c.fillText(`atk:${kind} ${Math.round(line.left)}..${Math.round(line.right)} ${mode}`,labelX,labelY);}const ro=Math.round(actor.visualRenderOffsetWorldPx||0);const crowdX=Math.round(actor.visualCrowdFanoutPx||0);const crowdY=Math.round(actor.visualCrowdYOffsetPx||0);c.fillStyle='#f8fafc';c.fillText(`st:${actor.state} hp:${Math.round(actor.hp||0)} dist:${Math.round(actor.debugDistance?.combatBodyDistance||0)} range:${Math.round(actor.detectionRangePx||0)}`,box.left,box.top-18);c.fillText(`pos:${Math.round(posX)} x:${Math.round(actorX)} ro:${ro} crowd:${crowdX}/${crowdY}`,box.left,box.top-4);}for(const base of bases){const box=this.getDebugCombatBodyBox(base);if(!box)continue;c.strokeStyle='#a78bfa';c.strokeRect(box.left,box.top,box.right-box.left,box.bottom-box.top);}c.fillStyle='#f8fafc';c.fillText('white=combatX, magenta=render origin, cyan=crowd visual, green=visual-only, yellow=attack',20,356);c.restore();}
  drawHud(c, scene, debug) { const dog = scene?.actors?.find((a) => a.side === 'dog-player'); const cat = scene?.actors?.find((a) => a.side === 'cat-enemy'); const dogBase=scene?.bases?.find((b)=>b.side==='dog-player'); const catBase=scene?.bases?.find((b)=>b.side==='cat-enemy'); const aliveDogs=(scene?.actors||[]).filter(a=>a.isAlive()&&a.side==='dog-player').length; const aliveCats=(scene?.actors||[]).filter(a=>a.isAlive()&&a.side==='cat-enemy').length; c.fillStyle = '#0008'; c.fillRect(14, 14, 860, 208); c.fillStyle = '#f8fafc'; c.font = '20px ui-sans-serif'; const version = BATTLE_CONFIG.version || '0.0.0'; c.fillText(`Wanko Battle v${version}`, 24, 40); c.font = '14px ui-monospace, monospace'; c.fillText(`dogBase HP:${dogBase?.hp ?? '-'} catBase HP:${catBase?.hp ?? '-'}`,24,62); c.fillText(`dog HP/state: ${dog?.hp ?? '-'} / ${dog?.state ?? '-'}`, 24, 84); c.fillText(`cat HP/state: ${cat?.hp ?? '-'} / ${cat?.state ?? '-'}`, 24, 104); c.fillText(`money:${Math.floor(scene?.economy?.money||0)}/${scene?.economy?.maxMoney||0} dogs:${aliveDogs} cats:${aliveCats}`,24,124); c.fillText(`battleState:${scene?.battleState || '-'} debug:${debug?.showBounds?'ON':'OFF'} effects:${(scene?.effects||[]).length}`,24,144); const sd=scene?.stage?.definition?.summary; c.fillText(`stage:${sd?.stageLen ?? '-'} bg:${sd?.bgId ?? '-'} baseHp:${sd?.enemyBaseHp ?? '-'} maxEnemy:${sd?.maxEnemyCount ?? '-'}`,24,164); const sp=scene?.stage?.spawnPreview?.summary; const firstMs=Number.isFinite(sp?.firstSpawnMs)?sp.firstSpawnMs:'-'; c.font='13px ui-monospace, monospace'; c.fillText(`stageSpawn rows:${sp?.totalRows ?? '-'} boss:${sp?.bossRows ?? '-'} unresolved:${sp?.unresolvedRows ?? '-'} first:${firstMs}ms`,24,184); }
  drawHpBar(c, actor) { const yOffset = BATTLE_CONFIG.visualLayout?.actorHpBarYOffset ?? 194; const visualX=actor.x+(actor.visualRenderOffsetWorldPx||0)+(actor.visualCrowdFanoutPx||0); const visualY=actor.y+(actor.visualCrowdYOffsetPx||0); const x = visualX - 40, y = visualY - yOffset, w = 80, h = 8; const ratio = Math.max(0, Math.min(1, actor.hp / Math.max(1, actor.maxHp))); c.fillStyle = '#111827'; c.fillRect(x, y, w, h); c.fillStyle = '#22c55e'; c.fillRect(x, y, w * ratio, h); c.strokeStyle = '#e5e7eb'; c.strokeRect(x, y, w, h); }
  drawActorDebug(c, actor, battleState) { const d = actor.debugDistance || {}; const lifeState = actor.isAlive?.() ? 'alive' : (actor.isFinalKnockback?.() ? 'finalKB' : actor.state); const frontScreenOffset = Number.isFinite(actor.visualRenderOffsetDebug?.frontScreenOffset) ? Math.round(actor.visualRenderOffsetDebug.frontScreenOffset) : '-'; const mode = actor.combatPositionMode || BATTLE_CONFIG.tuning?.combatPositionMode || 'screen-combat-point'; const combatX = Math.round(BattleBodyResolver.getActorCombatPositionX(actor)); const cap=actor.lastCaptureDebug?.capturedCount; const q=actor.lastHitQueueDebug?.damage; const crowdX=Math.round(actor.visualCrowdFanoutPx||0); const crowdY=Math.round(actor.visualCrowdYOffsetPx||0); const lines = [`${actor.instanceId||'-'} ${actor.state} hp:${Math.round(actor.hp||0)}`,`combatX:${combatX} range:${Math.round(actor.detectionRangePx||0)} dist:${Math.round(d.combatBodyDistance??0)} can:${d.canAttack===undefined?'-':d.canAttack}`,`cap:${cap===undefined?'-':cap} mode:${actor.lastCaptureDebug?.mode||mode} q:${q===undefined?'-':q} ro:${Math.round(actor.visualRenderOffsetWorldPx||0)} crowd:${crowdX}/${crowdY}`]; c.fillStyle = '#0008'; c.fillRect(actor.x - 170, actor.y - 190, 560, 48); c.fillStyle = '#f8fafc'; c.font = '12px ui-monospace, monospace'; lines.forEach((line, i) => c.fillText(line, actor.x - 164, actor.y - 176 + i * 14)); }
  ...
  drawEventLog(c, events){c.fillStyle='#0008';c.fillRect(14,170,900,170);c.fillStyle='#fde68a';c.font='13px ui-monospace';c.fillText('battle events (latest 10)',24,190);c.fillStyle='#f8fafc';(events||[]).slice().reverse().forEach((e,i)=>{const actor=e.actor||'-'; const target=e.target?` -> ${e.target}`:''; const dmg=e.damage!==undefined?` dmg:${e.damage}`:''; const cnt=e.count!==undefined?` count:${e.count}`:''; const mode=e.mode?` ${e.mode}`:''; c.fillText(`${Math.round(e.timeMs)} ${e.type} ${actor}${target}${cnt}${dmg}${mode}`,24,210+i*14);});}
  ...
  drawActor(c, actor) {
    ...
    c.translate(actor.x + modelAlignOffsetX + crowdOffsetX, actor.y + crowdOffsetY);
    if (actor.renderFlipX) c.scale(-1, 1);
    const s = Number.isFinite(actor.scale) ? actor.scale : 1;
    c.scale(s * crowdScale, s * crowdScale);
