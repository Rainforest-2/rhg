...
export const BCU_BURROW_ANIMATION_IDS = Object.freeze({ down: 'anim04', move: 'anim05', up: 'anim06' });
export const BCU_BURROW_ANIMATION_ROLES = Object.freeze({ down: 'burrow-down', move: 'burrow-move', up: 'burrow-up' });
...
function direction(actor) { return Number(actor?.direction) < 0 ? -1 : 1; }
function animationMapHas(container, id) { if (!container || !id) return false; if (container instanceof Map) return container.has(id); return !!container[id]; }
function templateHasLoadedAnimation(template, id) { if (!template || !id) return false; if (template.loadedAnimations?.has?.(id)) return true; return animationMapHas(template.animations, id); }
...
export function getMissingBcuBurrowAnimationIds(actorOrTemplate) {
  return getRequiredBcuBurrowAnimationIds().filter((id) => {
    if (actorOrTemplate?.loadedAnimations || actorOrTemplate?.animations && !(actorOrTemplate?.animations instanceof Map)) return !templateHasLoadedAnimation(actorOrTemplate, id);
    return !animationMapHas(actorOrTemplate?.animations, id);
  });
}
export function hasBcuBurrowAnimationSet(actor) { return getMissingBcuBurrowAnimationIds(actor).length === 0; }
export function hasBcuBurrowTemplateAnimations(template) { return getMissingBcuBurrowAnimationIds(template).length === 0; }
...
function isAliveForBurrow(actor) { if (typeof actor?.isAlive === 'function') return actor.isAlive() || actor?.state === 'burrow'; return actor?.hp > 0 && actor?.state !== 'dead'; }
...
function phaseFrames(actor, phase) {
  ...
  const maxFrame = Number(actor?.animations?.get?.(BCU_BURROW_ANIMATION_IDS[phase])?.maxFrame);
...
function setBurrowAnimation(actor, phase, restart = true) {
  ...
  actor.animator?.setLoop?.(phase === 'move');
  actor.lastBcuBurrowAnimationDebug = { source: 'BcuBurrowLifecycleRuntime.setBurrowAnimation', phase, animId, role, loaded: actor.animations?.has?.(animId) === true };
}
function advanceBurrowAnimationFrame(actor) {
  actor.animator?.tick?.(BCU_BATTLE_TIMER_PERIOD_MS);
...
function transition(actor, phase, scene, { skip = true } = {}) {
  ...
  if (phase === 'move') { actor.bcuBurrow.framesRemaining = 0; actor.bcuBurrow.distanceRemaining = getBcuBurrowSpec(actor)?.dis ?? actor.bcuBurrow.distanceRemaining; setBurrowAnimation(actor, 'move', true); }
  else if (phase === 'up') { actor.bcuBurrow.framesRemaining = phaseFrames(actor, 'up') + (skip === false ? 1 : 0); setBurrowAnimation(actor, 'up', true); }
...
export function finishBcuBurrow(actor, reason = 'complete') {
  ...
  actor.bcuBurrow.active = false; actor.bcuBurrow.phase = 'normal'; actor.bcuBurrow = null;
  actor.setState?.('move'); actor.setAnimation?.(actor.moveAnimId, 'move', true);
}
export function clearBcuBurrow(actor, reason = 'clear') { if (!actor?.bcuBurrow) return { cleared: false }; finishBcuBurrow(actor, reason); return { cleared: true, reason }; }
export function getBcuBurrowTouchMask(actor) { const phase = actor?.bcuBurrow?.phase; if (phase === 'down' || phase === 'up') return BCU_TOUCH_NORMAL | BCU_TOUCH_UNDERGROUND; if (phase === 'move') return BCU_TOUCH_UNDERGROUND; return BCU_TOUCH_NORMAL; }
export function isBcuBurrowNormallyTargetable(actor) { return getBcuBurrowTouchMask(actor) !== BCU_TOUCH_UNDERGROUND; }
export function isBcuBurrowTargetableForEvent(actor, event = null) { const mask = getBcuBurrowTouchMask(actor); const attackMask = Number(event?.bcuTouchMask ?? event?.touchMask ?? BCU_TOUCH_NORMAL); return (mask & attackMask) !== 0; }

function isSlowActive(actor, scene) { return scene?.isActorBcuSlowed?.(actor) === true || actor?.isBcuProcStatusActive?.('slow', scene?.timeMs) === true || actor?.bcuProcStatuses?.slow?.framesRemaining > 0; }
function rawSpeed(actor) {
  ...
  return Number.isFinite(perSecond) ? perSecond / fps / 0.5 : 0;
}
function globalSpeed(scene, actor, speed) {
  const fn = scene?.getBcuGlobalSpeed; if (typeof fn !== 'function') return speed; const dir = direction(actor);
  for (const call of [() => fn(dir, speed), () => fn(actor, speed), () => fn(actor?.side, speed)]) { try { const v = Number(call()); if (Number.isFinite(v) && v > -1) return v; } catch {} }
  return speed;
}
function speedProc(actor) { return actor?.bcuMoveSpeedProc || actor?.bcuProcStatuses?.speed || null; }
function speedupPercent(actor) { const n = Number(actor?.bcuSpeedupPercent ?? actor?.bcuProcStatuses?.speedup?.percent ?? actor?.bcuProcStatuses?.strengthenSpeed?.percent); return Number.isFinite(n) ? n : 0; }
export function getBcuBurrowMoveDistancePerFrame(actor, scene = null) {
  if (isSlowActive(actor, scene)) return 0.25;
  let mov = globalSpeed(scene, actor, rawSpeed(actor)) * 0.5;
  const sp = speedProc(actor);
  if (sp && Number(sp.framesRemaining ?? sp.durationFrames ?? 0) !== 0) { const mode = Number(sp.mode ?? sp.type ?? 0); const amount = Number(sp.amount ?? sp.value ?? sp.percent ?? 0) || 0; if (mode === 0) mov += amount * 0.5; else if (mode === 1) mov = mov * (100 + amount) / 100; else if (mode === 2) mov = amount * 0.5; }
  const up = speedupPercent(actor); if (up !== 0) { mov += mov * up / 100; mov = Math.round(mov * 4) / 4; }
  return mov;
}
function moveBcuBurrowFrame(actor, scene) { const before = Number(actor.x) || 0; const moved = getBcuBurrowMoveDistancePerFrame(actor, scene) * direction(actor); actor.x = before + moved; actor.posBcu = actor.x; return actor.x - before; }
...
export function tickBcuBurrow(actor, dt = BCU_BATTLE_TIMER_PERIOD_MS, { scene = actor?.scene || null } = {}) {
  ...
  if (!isAliveForBurrow(actor)) { clearBcuBurrow(actor, 'death'); return { active: false, cleared: true }; }
  if (scene?.isActorBcuStopped?.(actor) === true || actor?.isBcuProcStatusActive?.('freeze', scene?.timeMs) === true) { actor.lastBcuBurrowTickDebug = { source: 'BcuBurrowLifecycleRuntime.tick', frozen: true, phase: state.phase }; advanceBurrowAnimationFrame(actor); return { active: true, frozen: true, state }; }
  ...
  for (let i = 0; i < steps && actor.bcuBurrow?.active; i += 1) {
    if (state.phase === 'down') { state.framesRemaining -= 1; if (state.framesRemaining <= 0) transition(actor, 'move', scene); }
    if (state.phase === 'move') { const base = baseAhead(scene, actor); if (state.distanceRemaining < 0) transition(actor, 'up', scene, { skip: false }); else if (base.base && base.distance - base.touchBase <= 0) transition(actor, 'up', scene, { skip: true }); else { const moved = moveBcuBurrowFrame(actor, scene); movedTotal += moved; state.distanceRemaining -= moved * direction(actor); } }
    if (state.phase === 'up') { state.framesRemaining -= 1; if (state.framesRemaining <= 0) finishBcuBurrow(actor, 'up-complete'); }
    advanceBurrowAnimationFrame(actor);
  }
  actor.lastBcuBurrowTickDebug = { source: 'BcuBurrowLifecycleRuntime.tick', active: !!actor.bcuBurrow?.active, phase: actor.bcuBurrow?.phase || 'normal', x: actor.x, moved: movedTotal, distanceRemaining: actor.bcuBurrow?.distanceRemaining ?? null, movePerFrame: getBcuBurrowMoveDistancePerFrame(actor, scene), animationFrame: actor.animator?.frame ?? null };
