...
export class BcuEnemyRepository {
  ...
  async build() {
    ...
    let rows = [];
    try { rows = parseCsvRows(await this.readText(statsPath)).map(toNumbers); } catch (error) { this.diagnostics.enemies.missingStats.push({ file: statsPath, reason: error?.message || String(error) }); }
    for (const id of this.manifest.indexes?.enemyIds || []) {
      const enemyId = toInt(id, null); if (!Number.isFinite(enemyId)) continue;
