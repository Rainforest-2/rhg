export class BcuModelInstance {
  constructor(model){
    ...
    this.parts=model.parts.map((p)=>({...p,base:{...p,hf:1,vf:1,gsca:this.baseScale},current:{...p,hf:1,vf:1,gsca:this.baseScale}}));
  }
  ...
  applyTrack(partId, prop, v, modification=null){ const p=this.parts[partId]; if(!p){ this.lastAppliedTrackDebug={partId,prop,modification,applied:false,value:v}; return {applied:false}; } const b=p.base,c=p.current; const bs=this.baseScale,bo=this.baseOpacity; switch(modification){case 0:c.parent=Math.trunc(v);break;case 1:c.imgcutIndex=Math.trunc(v);break;case 2:c.partIndex=Math.trunc(v);break;case 3:c.zOrder=Math.trunc(v);break;case 4:c.posX=b.posX+v;break;case 5:c.posY=b.posY+v;break;case 6:c.pivotX=b.pivotX+v;break;case 7:c.pivotY=b.pivotY+v;break;case 8:c.scaleX=b.scaleX*v/bs;c.scaleY=b.scaleY*v/bs;break;case 9:c.scaleX=b.scaleX*v/bs;break;case 10:c.scaleY=b.scaleY*v/bs;break;case 11:c.angle=b.angle+v;break;case 12:c.opacity=v*b.opacity/bo;break;case 13:c.hf=v===0?1:-1;break;case 14:c.vf=v===0?1:-1;break;case 53:c.gsca=v;break;default: if(prop==='posX')c.posX=b.posX+v; else if(prop==='posY')c.posY=b.posY+v; else if(prop==='partIndex')c.partIndex=Math.trunc(v);} const result={applied:true,partId,modification}; this.lastAppliedTrackDebug={partId,prop,modification,applied:true,value:v}; return result; }
  ...
  getBattleDrawList({parentMatrix=null}={}){
    ...
    const drawList=this.parts.map((part)=>{ const c=this.getPartCurrent(part); const graphicsMatrix=this.getPartGraphicsMatrix(part,parentMatrix,gCache,sCache,bCache); const size=this.getPartSize(part,sCache); const drawMatrix=this.multiplyMatrix(graphicsMatrix,this.scaleMatrix(size.x,size.y)); const opacity=this.getPartOpacity(part,oCache); return {index:part.index,partIndex:c.partIndex,imgcutIndex:c.imgcutIndex,opacity,matrix:drawMatrix,graphicsMatrix,bcuSize:size,pivotX:c.pivotX,pivotY:c.pivotY,rawPart:part,z:(c.zOrder??part.zOrder??0)*Math.max(1,this.parts.length)+part.index}; }).sort((a,b)=>a.z-b.z); const visibleCount=drawList.filter((d)=>d.opacity>0&&Number.isFinite(d.partIndex)&&Number.isFinite(d.imgcutIndex)).length; const zValues=drawList.map((d)=>d.z).filter(Number.isFinite); this.lastDrawListDebug={source:'BcuModelInstance.getBattleDrawList',count:drawList.length,visibleCount,opacityZeroCount:drawList.filter((d)=>d.opacity<=0).length,minZ:zValues.length?Math.min(...zValues):null,maxZ:zValues.length?Math.max(...zValues):null,hasMatrix:drawList.some((d)=>Array.isArray(d.matrix)&&d.matrix.length===6),examples:drawList.slice(0,3).map((d)=>({index:d.index,partIndex:d.partIndex,imgcutIndex:d.imgcutIndex,z:d.z,opacity:d.opacity,matrix:Array.isArray(d.matrix)?d.matrix.slice(0,6):null}))}; return drawList;
  }
