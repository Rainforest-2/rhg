export class BattleCameraInputController {
  ...
  onMove(e){ if(!this.active.has(e.pointerId)) return; const p=this.active.get(e.pointerId); p.px=p.x; p.py=p.y; p.x=e.clientX; p.y=e.clientY; const cam=this.getCamera?.(); if(!cam) return;
    ...
    if(pts.length>=2){ const [a,b]=pts; const prev=Math.hypot(a.px-b.px,a.py-b.py); const cur=Math.hypot(a.x-b.x,a.y-b.y); if(prev>0&&cur>0){ const center=(a.x+b.x)*0.5; cam.zoomAtScreenPoint(center, (cam.siz??cam.zoom??1)*(cur/prev)); } }
  }
  onUp(e){ this.active.delete(e.pointerId); }
  onWheel(e){ const cam=this.getCamera?.(); if(!cam) return; e.preventDefault(); if(e.ctrlKey||e.metaKey){ cam.zoomAtScreenPoint(e.clientX, (cam.siz??cam.zoom??1)*(e.deltaY<0?1.08:0.92)); } else { cam.panByScreenDelta(-(e.deltaX||e.deltaY)); } }
}
