...
export class FormationEditor {
  ...
  refresh() {
    ...
    this.root.innerHTML = `<div class='formation-panel'><h3>編成</h3>
      ...
      <div class='formation-catalog-grid'>${chars.map((c)=>`<button class='formation-character-card ${usedBaseIds.has(getCharacterBaseId(c))?'is-used':''}' data-character='${c.characterId}'><img src='${c.uiIcon.primary}' onerror="this.onerror=null;this.src='${c.uiIcon.fallback}'"><small>${c.factionLabel}</small><span>${c.label}</span>${this.renderCharacterStats(c)}</button>`).join('')}</div>
