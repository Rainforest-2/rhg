...
export class PlayerProductionBar {
  ...
  async ensureCardAssets(unitDef) { if (!unitDef?.uiIcon) return { icon: null, failed: false }; const key = [unitDef.characterId,unitDef.slotId,unitDef.assetId,unitDef.uiIcon.primary,unitDef.uiIcon.fallback].filter(Boolean).join('|'); if (this.iconCache.has(key)) return this.iconCache.get(key); const p = (async () => { try { return { icon: await loadImage(unitDef.uiIcon.primary), failed: false }; } catch (e1) { this.cardSkin.log.warn?.('[PlayerProductionBar] primary icon load failed', unitDef.uiIcon.primary, e1); if (unitDef.uiIcon.fallback) { try { return { icon: await loadImage(unitDef.uiIcon.fallback), failed: false }; } catch (e2) { this.cardSkin.log.warn?.('[PlayerProductionBar] fallback icon load failed', unitDef.uiIcon.fallback, e2); } } return { icon: null, failed: true }; } })(); this.iconCache.set(key, p); return p; }
