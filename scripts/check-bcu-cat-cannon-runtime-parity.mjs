...
const chargeEvent = scene.debugEvents.find((event) => event.type === 'bcuCatCannonCharged');
assert.equal(chargeEvent?.cannonId, 0, 'cat cannon charge-ready event must carry the BCU cannon id for SE_CANNON_CHARGE');
assert.equal(chargeEvent?.beforeCannon, 2, 'cat cannon charge-ready event must capture the pre-fill cannon value');
assert.equal(chargeEvent?.afterCannon, 3, 'cat cannon charge-ready event must capture the max cannon value');
