...
// 120 has BLEND glow parts in its initial pose, so it is regenerated by
// repair-bcu-enemy-icon-glow-zip.mjs and can no longer be a byte-stable witness.
const UNTOUCHED_IDS = Object.freeze([100, 101, 121]);
// Initial poses with BCU BLEND glow parts (user-visible black-box regressions).
// Their zip bytes must match the current generator output exactly.
const GLOW_COMPOSED_IDS = Object.freeze([179, 236, 248]);
...
for (const id of GLOW_COMPOSED_IDS) {
  const key = `enemy:${id}`;
  const internalPath = `enemy/${pad3(id)}.png`;
  const data = zip.get(internalPath);
  assert.ok(data, `${internalPath} exists in enemy.zip`);
  assertPng512(data, internalPath);
  const actorEntry = actorByKey[key];
  ...
  assert.equal(generated.status, 'generated', `${key} generation status`);
  assert.equal(generated.compositionMethod, 'composed-initial-pose', `${key} composition method`);
  assert.equal(sha256(data), generated.sha256, `${internalPath} matches glow-aware composed icon`);
}
